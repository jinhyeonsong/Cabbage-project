	...
	public static String getPaginate(int pageNo,
		...
		if(total!=0) {
			...
			if(pageNo<=1) {
				...
				//활성화
				paginate += "<a href='"+url+"?"+param+(pageNo-1)+"' data-page='"+(pageNo-1)+"' title='이전 페이지로'><i class='fa fa-chevron-left'></i><span class='screen_out'>이전 페이지</span></a>";
			...
			for(int i = 1 ; i <= numBlock ; i++) {
				...
				if(realPage==pageNo) {
					...
					//현재 페이지가 아님
					paginate += "<a href='"+url+"?"+param+realPage+"' data-page='"+realPage+"' title='"+realPage+"'>"+realPage+"</a>";
			...
			if(pageNo >= totalPage) {
				...
				//활성화
				paginate+="<a href='"+url+"?"+param+(pageNo+1)+"' data-page='"+(pageNo+1)+"' title='다음 페이지로'><i class='fa fa-chevron-right'></i><span class='screen_out'>다음 페이지</span></a>";
			}//if end

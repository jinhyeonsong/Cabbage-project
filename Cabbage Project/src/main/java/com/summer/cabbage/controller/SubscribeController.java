...
public class SubscribeController {
	...
	public String subscribeDetail(Model model,@PathVariable int productNo) {
	...
	// 0314 수정 / 03-04 이소현 _ 카테고리 있는 메인 목록 페이지 (2차 카테고리 목록하고 지역(시도)받아옴
	@RequestMapping(value = "/{category}", method = RequestMethod.GET)
	public String mainList(@PathVariable String category, Model model) {

		// System.out.println(category);

		model.addAllAttributes(service.getSecondCategoryList(category));

		return "category_list";

	}

	// 0312 이소현 _ (목록페이지) 2차 카테고리 탭 클릭시 ajax로 목록 받아오기
	@RequestMapping(value = "/ajax/categoryNo", method = RequestMethod.GET, produces = "application/json;charset=UTF-8")
	@ResponseBody
	public Map<String, Object> productList(int categoryNo, int page, String sort) {

		return service.getProductList(categoryNo, page, sort);
	}

	// 0313 이소현 _ (목록 페이지) 지역 받아오기
	@RequestMapping(value = "/ajax/region", method = RequestMethod.GET, produces = "application/json;charset=UTF-8")
	@ResponseBody
	public List<Region> regionList(int stateNo) {
	...
	public String searchResult(Model model, @PathVariable String keyword) {
		...
		System.out.println("컨트롤러에서" + keyword);

		model.addAttribute(service.getSearchResult(keyword));

		return "search_result";
	}

	// 0315 이소현 _ (검색결과) 목록 ajax 로 나타내기
	@RequestMapping(value="/ajax/{keyword}", method = RequestMethod.GET)
	@ResponseBody
	public Map<String, Object> resultList(@PathVariable String keyword, int page, String sort, String category) {
			
		return service.getResultProductList(keyword, page, sort, category);
	}
	...
	private String uploadSubs(HttpSession session, Product product, String editorContent, int[] days,
			int[] deliveryAvailSubState, int[] deliveryAvailTax) {
		...
		boolean result = service.registerProduct(loginMember, product, editorContent, days, deliveryAvailSubState,
				deliveryAvailTax);

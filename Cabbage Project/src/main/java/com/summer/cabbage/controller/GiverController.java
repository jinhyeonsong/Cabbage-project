...
public class GiverController {
	...
	@RequestMapping(value = "/giver/{no}/personalInfo", method = RequestMethod.GET)
	...
	//03-16 이아림 추가 
    //기버마이페이지 연락처 수정
   @RequestMapping(value="/ajax/giver/{no}/phone", method=RequestMethod.POST, produces="application/json;charset=UTF-8")
   @ResponseBody    
   public String updatePhone(Giver giver, @PathVariable int no) {
	   giver.setNo(no);
	   System.out.println(giver.getPhone()+"controller");
	   service.updatePhone(giver);
	   return "{\"phone\":\""+giver.getPhone()+"\"}";
   }//checkPhone() end
   //03-16 이아림 추가 end
   
   //03-12 방 상품관리
   
   @RequestMapping(value="/giver/{giverNo}/products", method=RequestMethod.GET)
	public String productManagementInfo(@PathVariable int giverNo, Model model) {
		
		// 각 태그별 개수 현황 받아오기
		model.addAllAttributes(service.getItemNumbersByType(giverNo));
		
		return "productManagement";
	}
	
	// 03-12 방 상품 목록 ajax
	@RequestMapping(value="/ajax/totalProductList", method=RequestMethod.GET)
	@ResponseBody
	public List<Product> getTotalProductList(Product product){
		
		return service.getTotalProductList(product);
	}

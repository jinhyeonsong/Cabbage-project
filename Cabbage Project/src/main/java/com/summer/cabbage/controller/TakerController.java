...
public class TakerController {
	...
	// 0311 리뷰 작성 : 오승주
	@RequestMapping(value = "/taker/review", method = RequestMethod.POST)
	public String writeReview(Review review) {
		// 데이터 베이스에리뷰 등록 하기
		service.reviewRegister(review);
		return "redirect:/taker/" + review.getTakerNo();
	}

	// 0311 구독 취소 : 오승주
	@RequestMapping(value = "/taker/subscription/{subsNo}", method = RequestMethod.PUT)
	public String cancelSub(@PathVariable int subsNo, @RequestHeader String referer) {
		// subsNo를 활용하여 구독 테이블에서 해당하는 튜플을 제거한다.
		service.cancelSub(subsNo);
		return "redirect:" + referer;
	}

	// 0314 리뷰 수정
	@RequestMapping(value = "/taker/review", method = RequestMethod.PUT)
	...
	// 리뷰 수정 시 기존 데이터 불러오기 위한 ajax
	@RequestMapping(value = "/review", method = RequestMethod.GET)
	@ResponseBody
	public Review getReviewData(Review review) {
		return service.getReviewData(review);
	}

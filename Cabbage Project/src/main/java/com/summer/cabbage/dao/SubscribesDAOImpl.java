...
public class SubscribesDAOImpl implements SubscribesDAO {
	...
	// 0305 추가 : 오승주
	@Override
	public Date selectThisPayment(int no) {
		// TODO Auto-generated method stub

		return session.selectOne("subscribes.selectThisPayment", no);
	}

	// 0311 구독 취소 : 오승주
	@Override
	public void updateSub(int subsNo) {
		// TODO Auto-generated method stub
		session.update("subscribes.updateSub", subsNo);
	}
	// 03-09 방 추가

	@Override
	public int insertSubscribe(Subscribe subscribe) {
		return session.insert("subscribes.insertSubscribe", subscribe);
	}

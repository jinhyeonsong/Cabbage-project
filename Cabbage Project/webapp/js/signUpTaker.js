...
function checkNickname(nickname){
    let nicknameEng = nickname.search(/[a-z]/ig);
    let nicknameKor = nickname.search(/[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/i);

    if(nickname>10 || nicknameEng<0 || nicknameKor<0){
        $nicknameNotice.show();
        $nicknameNotice.css("color", "crimson");
        $nicknameNotice.children("span").text("닉네임은 한/영 10자까지 가능합니다.");
    }else {
        /*$.ajax({
            url: "뭐시기",
            type: "GET",
